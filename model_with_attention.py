...
class ReconstructiveSubNetwork_Chattention_Refine(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, base_width=64):
        super(ReconstructiveSubNetwork_Chattention_Refine, self).__init__()
        self.encoder = EncoderReconstructive_Cha(in_channels, base_width)
        self.decoder = DecoderReconstructive_Cha(base_width, out_channels=out_channels)
        self.refine_network = RefineNet(in_channels=3, out_channels=2, base_width=16)

    def forward(self, x):
        b5, feas = self.encoder(x)
        output = self.decoder(b5)
        # 初始化 refined_outputs 列表
        refined_outputs = []
        for i in range(4):
            input_single_channel = x[:, i:i+1, :, :]  # B * 1 * H * W
            output_two_channels = output[:, 2*i:2*i+2, :, :]  # B * 2 * H * W
            refine_input = torch.cat([input_single_channel, output_two_channels], dim=1)  # B * 3 * H * W
            refined_output = self.refine_network(refine_input)  # B * 2 * H * W
            refined_outputs.append(refined_output)
        # 将所有 refined_output 在通道维度上拼接
        refined_output = torch.cat(refined_outputs, dim=1)  # B * 8 * H * W
        return output, feas, refined_output


class RefineNet(nn.Module):

    def __init__(self, in_channels=3, out_channels=2, base_width=16):
...
class DecoderReconstructive_CBAM(nn.Module):
    ...
    def __init__(self, base_width, out_channels=1):
        ...
        self.fin_out = nn.Sequential(nn.Conv2d(base_width, out_channels, kernel_size=1))
...
class EncoderReconstructive_Spa(nn.Module):
    def __init__(self, in_channels, base_width):
        ...
        self.block2 = nn.Sequential(
            ...
            nn.BatchNorm2d(base_width*2),
            nn.ReLU(inplace=True))
        self.attention2 = SpatialAttention_WH(in_channel=base_width*2, inter_channel=base_width, subsample_rate=4)
        self.mp2 = nn.Sequential(nn.MaxPool2d(2))
        self.block3 = nn.Sequential(
            nn.Conv2d(base_width*2, base_width*4, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width*4),
...
class DecoderReconstructive_Cha(nn.Module):
    ...
    def __init__(self, base_width, out_channels=1):
        ...
        self.fin_out = nn.Conv2d(base_width, out_channels, kernel_size=1)
